...
function checkDatabase() {
  ...
  // Open a transaction on your offline db
  let transaction = db.transaction(["offline"], "readwrite");

  // access your offline object
    ...
    if (getAll.result.length > 0) {
      fetch("/api/transaction/bulk", {
        ...
        .then((res) => {
          ...
          if (res.length !== 0) {
            // Open another transaction to offline with the ability to read and write
...
function saveRecord(record) {
  ...
  // Create a transaction on the offline db with readwrite access
